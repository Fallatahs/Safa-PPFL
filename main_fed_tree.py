...
# converting main_fed_serial to a binary tree structure - random 
# with no changes in the aggregation function FedAvg_serial(w_serial) 
# check masking logic
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        # Extract node values from idxs_users and create chain format
        ...
        # Convert idxs_users list to strings and then create a binary tree
        # root = build([str(user_idx) for user_idx in idxs_users], 0)

        # Convert the user indices to integers and build a binary tree
        root = build([int(user_idx) for user_idx in idxs_users])

        #visual representation2
    ...
    lossfile = open('./log/lostfile_{}_{}_{}_iid{}_serial.dat'.format(args.dataset, args.model, args.epochs, args.iid), "w")
    ...
    plt.savefig('./log/fed_serial_rand_{}_{}_{}_C{}_iid{}_serial.png'.format(args.dataset, args.model, args.epochs, args.frac, args.iid))
